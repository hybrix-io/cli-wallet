...
} else if (ops.allocationCreate || ops.allocationAddress || ops.allocationPairGet || ops.allocationPairSet || ops.allocationWithdrawSet) {
  // allocation account functionality (start)
  if (ops.allocationAddress) {
    const aSymbol = ops.allocationAddress;
    progress.push('allocationAddress');
    actions.push(
      {host: hostnameAllocations}, 'addHost',
      {username: userid, password: passwd, offset: accountOffset}, 'session',
      {symbol: aSymbol}, 'addAsset',
      {symbol: aSymbol}, 'getAddress'
    );
  } else {
    actions.push(
      {host: hostnameAllocations}, 'addHost',
