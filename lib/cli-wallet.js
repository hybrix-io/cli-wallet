...
for (let id in methods) {
  if (ops[id]) actions.push(...methods[id][id](ops)(ops[id])); // TODO may pass more parameters?
}
...
if (ops.address) {
  ...
  actions.push({symbol: ops.address}, 'getAddress');
}

if (ops.publickey) {
  progress.push('publickey');
  actions.push({symbol: ops.publickey}, 'getPublicKey');
}

if (ops.secretkey) {
  progress.push('secretkey');
  actions.push({symbol: ops.secretkey}, 'getPrivateKey');
}

if (ops.keysobject) {
  progress.push('keys');
  actions.push({symbol: ops.keysobject}, 'getKeys');
}
