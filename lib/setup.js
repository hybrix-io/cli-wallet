...
function setupHost (ops, method, actions) {
  ...
  else if (ops.local) host = DEFAULT_HOSTNAME_ALLOCATIONS;
...
function setupSession (ops, method, actions) {
  ...
  if (ops.method === 'api' || (ops.module && method === 'help')) return; // nothing to do

  if (!login.username) login.username = readlineSync.question('Username: ');
  if (!login.password) login.password = readlineSync.question('Password: ', {hideEchoBack: true});
